...
async def test_execute_mode_skips_yaml_nudge(monkeypatch):
    ...
    async def mock_chat(messages, system_prompt, tools, dispatch_fn, max_rounds=30, on_stream=None):
        ...
        return "I executed the task. The title is Example Domain.", [], 1, {}
...
async def test_yaml_mode_triggers_nudge(monkeypatch):
    ...
    async def mock_chat(messages, system_prompt, tools, dispatch_fn, max_rounds=30, on_stream=None):
        ...
        call_count += 1
        if call_count == 1:
            return "Sure, I can help with that.", [], 1, {}  # No YAML → triggers nudge
        else:
...
async def test_yaml_mode_uses_default_prompt(monkeypatch):
    ...
    assert "GENERATION" in captured_prompt["value"]
    assert "DISCOVERY" in captured_prompt["value"]
