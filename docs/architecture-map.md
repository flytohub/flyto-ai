...
See [the runtime contract](local-cli-runtime.md) for limits and verification.
...
| Governed Robotics planner -> Blueprint/Core discovery | implemented (2026-08-22) | `flyto_ai/robotics_planner_server.py::GovernedRoboticsPlanner` requires goal-frame normalization and Blueprint/Core capability discovery before the provider-facing `RoboticsPlanningService`; routing refusal never reaches the provider. Robotics still owns executable validation and motion authority. |
...
| Indexer / Blueprint / Core surround every public audited coding job | implemented | `flyto_ai/coding/route.py` runs the mandatory Indexer pre/post lanes, Blueprint discovery, and Core validation around whichever implementer startup selected. All four lanes are configured on every strict public route; none is detachable. Blueprint and Core may resolve only `applied` or `not_applicable`. Every project-aware Indexer plan analysis (`search`, `impact`, `structure`, `call_hierarchy`) carries the host-derived workspace project; conflicting project evidence is rejected before analysis or editing, so an isolated worktree never falls through to ambient indexes. First-round path authority accepts both a strongly declared exact/only/each repository-relative set through a colon-delimited numbered list and an affirmative mutation colon heading that explicitly declares exact new files or paths followed only by comma- or semicolon-delimited whole paths. Generic report/evidence headings are masked context. An admitted headed statement is one ordered atomic group: negative/mixed polarity, command/evidence prose, unsafe/non-file spelling, symlink ancestry or final link, per-statement overflow, or insufficient remaining global capacity rejects every still-new member without ordinary-parser prefix recovery. Both forms retain canonical-path, typed-suffix, missing-parent containment and 64-target checks; ordinary requests remain compatible and audit rework keeps its stricter same-clause mutation projection. The Indexer preset and public route share a finite ten-minute transport bound so large or contended strict verification can finish while every gate remains fail-closed. The Claude service adapter accepts one already-authorized SDK JSON frame up to a fixed 8 MiB ceiling so a legitimate large Indexer result does not hit the SDK's 1 MiB default; legacy direct calls retain that default, and no tool or route authority is widened. The explicit `codex` backend starts a separate logged-in Codex CLI thread with user config/rules ignored, no MCP/plugins/web search, a scrubbed environment and a bounded workspace sandbox; the host still owns session binding, snapshots, checks, route lanes and the independent exact-revision audit. Its JSONL protocol permits one event up to 2 MiB under a separate 8 MiB whole-stream ceiling and records only content-free size/counter diagnostics. A valid bounded `turn.completed` event closes the model turn even if later CLI teardown exits non-zero; that exit stays in evidence, while missing completion, malformed or oversized output, timeout, failed checks and no-change work remain fail-closed. One execution plan may select the published legacy (`assess` / `implement`) or current (`plan_changes` / `apply_changes`) Indexer gate family; unknown, duplicate, or mixed-family plans fail before dispatch. On rework, the authenticated parent retains cumulative scope; the host sends Indexer only prior attributable paths absent from that ledger plus filesystem-validated explicit audit targets, or one authenticated parent path for a same-scope repair. Indexer forms the exact ordered cumulative union, so its 32-target per-amendment bound no longer rejects a valid 64-path root task. The stdlib-only amendment domain verifies Indexer's parent/content/entry digests, ids, complete linkage, normalized host project, profile/context mirrors and exact path partition, then reuses only an exact parent step multiset occurrence; missing instruction context fails closed, novel original-path analysis and every successor gate execute. During the ledger-schema transition, parent and successor independently accept only the historical `task-context.v1` or canonical `intent-ledger.v1` ledger label; instruction context remains exactly `task-context.v1`, and every unknown label still closes the lane. The pinned verifier's effective compatibility ceiling is generation seven, and every executable delta remains inside the ordinary step bound. Structured Indexer failures preserve only exact host-registered machine reasons/actions, never unknown tokens, prose, paths or secrets. Post validation accepts the legacy Boolean verdict or the current `overall=pass` envelope only when both bounded ruff/pytest statuses agree. Blueprint reuse requires token overlap and ranks ordered phrase overlap before catalogue order, so a direction-bearing transform is not replaced by its reverse. Proved by `tests/test_coding_route.py` against the installed Indexer end to end, the real Core adapter (`array.join` validated), and the real Blueprint adapter (`ConvertCSVtoJSON` projected), plus `tests/test_codex_cli_backend.py` for the Codex process boundary. |
| Long-lived Codex MCP process -> current coding worker | implemented | `flyto_ai/coding/mcp_supervisor.py` keeps host stdio stable, detects coding-source build drift, and preserves non-terminal work owned by that connection: its successful submits plus a correctly addressed successful `flyto_coding_audit(verdict=rework)` returning the same job in `rework_queued` or `rework_running`. A newly spawned worker has one 120-second first-response bound; state-bearing calls are refused until initialize succeeds, that response is structurally validated, and hot-reload replay must match both its exact result and the validated deterministic `tools/list` result. Valid initialize errors pass through without replacing the replay contract. Every later worker response retains the 30-second bound and uncertain calls are never retried. An initialized exact-shape `flyto_coding_get` may instead read one authenticated tenant/job record through the bounded no-follow `coding/fast_get.py` boundary, reusing canonical receipt, continuation, strict-route and emergency validation without writes or history enumeration. Terminal Mission reconciliation, unsafe storage, future argument shapes, incompatible backend/emergency startup selection, and source drift use the worker; detected drift permanently disables the imported fast validator until supervisor replacement. The `flyto_ai.coding` facade is lazy while `Agent`/`AgentConfig` remain eager for runtime typing compatibility; `coding/startup_reconciliation.py` resolves terminal Mission rows through one bounded fully validated snapshot and retains live reclaim/coordinate checks. The bounded build digest includes both implementation adapters and its source inventory is self-covered by `coding/route_status.py`. Reads and other audit outcomes never adopt foreign work; matching terminal get/audit observation or durable state releases an existing pin before safe inner-worker replacement. `CodingService.submit` independently rejects a stale direct worker before mutation. |
| Tenant coding job -> compact conditional polling | implemented | `flyto_ai/coding/mcp_server.py` keeps a `job_id`-only call exactly compatible with the historical `ok` plus full `job` response; typed observation appears only after an explicit detail or conditional-read argument. The fixed `summary` projection uses `CodingService.get_summary`: one tenant-derived exact-path atomic read outside the shared coordination guard, with route authority still revalidated for audit-ready/landable states and no reconciliation or dispatch authority. Its detail-scoped change token hashes only the selected redacted projection under a separate domain. A positive wait requires summary plus the prior token, runs only for background-progress states, and is capped at 20 seconds beneath the supervisor's 30-second deadline; audit-ready, blocked, accepted and terminal states return their closed next action immediately. Result/route/mission/evidence/session/path/file/check/prose fields remain full-only. The public inventory remains exactly submit/get/audit. |
...
- `flyto_ai/coding/mcp_supervisor.py`: stable local coding-MCP host edge with a
  build-aware, safely replaceable `code-mcp` worker. Fresh-worker initialize is
  bounded at 120 seconds and exactly validated; every subsequent worker read is
  deadlined at 30 seconds. A missed steady deadline terminates the wedged worker
  without retrying the uncertain request, and hot-reload tracking self-heals
  from durable job records so a client that stops polling cannot pin reloads.
  Exact initialized get calls use `coding/fast_get.py` to validate and project
  one tenant/job record without enumerating durable history or writing state.
- `flyto_ai/coding/startup_reconciliation.py` and
  `flyto_ai/orchestration/mission_batch.py`: cold-start job reconciliation over
  one bounded, fully validated MissionStore snapshot rather than one complete
  database validation per terminal candidate. Reclaim remains lease/fence based.
